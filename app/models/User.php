...
class User
{
	...
	{
		$query = $db_con->prepare('UPDATE users SET user_email_verification_hash=:email_hash, user_email_verification_hash_datetime=NOW() WHERE user_id=:id');
	...
	public function get_last_user_by_user_registration_datetime($db_con)
	{
		$query = $db_con->prepare('SELECT user_registration_datetime FROM users ORDER BY user_registration_datetime DESC LIMIT 1');
		$query->execute();
		return $query->fetch();
	}

	public function count_users($db_con)
	{
		$query = $db_con->prepare('SELECT COUNT(*) AS users_amount FROM users');
		$query->execute();
		return $query->fetch();
	}
}
